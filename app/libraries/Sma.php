...
class Sma
{
    ...
    public function paid_supplier($paid_by = null, $purchase = false, $empty_opt = false)
    {
        $opts = '';
        if ($empty_opt) {
    ...
    public function formatMoney1($number, $symbol = false)
    {
        if ($symbol !== 'none') {
            $symbol = $symbol ? $symbol : $this->Settings->symbol;
        } else {
        ...
        if ($this->Settings->sac) {
            return
            $this->formatSAC1($this->formatDecimal($number)).
            ($this->Settings->display_symbol == 2 ? $symbol : '');
        }
        $decimals = $this->Settings->decimals;
        $ts = $this->Settings->thousands_sep == '0' ? ' ' : $this->Settings->thousands_sep;
    ...
    public function send_email($to, $subject, $message, $from = null, $from_name = null, $attachment = null, $cc = null, $bcc = null)
    {
        ...
        if ($domain != 'tecdiary.com') {
            ...
            $config['useragent'] = 'Stock Manager By Web AGency';
            ...
            if ($this->email->send()) {
                ...
                throw new Exception($this->email->print_debugger(['headers', 'subject']));
    ...
    public function formatSAC1($num)
    {
        $pos = strpos((string) $num, '.');
        if ($pos === false) {
            $decimalpart = '0';
